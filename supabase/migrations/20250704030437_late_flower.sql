/*
  ...
  1. Storage Policies
    - Create policies for property-images bucket with proper error handling
...
EXCEPTION 
  ...
  WHEN OTHERS THEN
    RAISE NOTICE 'Error creating policy "Public read access for property images": %', SQLERRM;
END $$;
...
EXCEPTION 
  ...
  WHEN OTHERS THEN
    RAISE NOTICE 'Error creating policy "Authenticated users can upload property images": %', SQLERRM;
END $$;
...
EXCEPTION 
  ...
  WHEN OTHERS THEN
    RAISE NOTICE 'Error creating policy "Users can update their own property images": %', SQLERRM;
END $$;
...
EXCEPTION 
  ...
  WHEN OTHERS THEN
    RAISE NOTICE 'Error creating policy "Users can delete their own property images": %', SQLERRM;
END $$;
...
EXCEPTION 
  ...
  WHEN OTHERS THEN
    RAISE NOTICE 'Error creating policy "Public bucket access": %', SQLERRM;
END $$;
