...
const openWhatsApp = () => {
  if (product.value?.profiles?.phone) {
    const message = `Hola, estoy interesado en tu producto "${product.value.title}" publicado en ColombiaMarket.`
...
const sendEmail = () => {
  if (product.value?.profiles?.email) {
    ...
    const body = `Hola,\n\nEstoy interesado en tu producto "${product.value.title}" publicado en ColombiaMarket.\n\nPor favor, contáctame para más información.\n\nGracias.`
